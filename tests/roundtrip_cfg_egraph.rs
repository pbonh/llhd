...
fn roundtrip_cfg_skeleton_egraph() {
    ...
    files.par_iter().for_each(|path| {
        ...
        let mut module = parse_module_unchecked(&contents)
            .unwrap_or_else(|err| panic!("parse failed for {}: {}", path.display(), err));

        for mut unit in module.units_mut() {
            unit.rebuild_skeleton_egraph().unwrap_or_else(|err| {
                panic!("rebuild cfg/egraph failed for {}: {}", path.display(), err)
            });
        }

        let roundtrip = write_module_string(&module);
        let roundtrip_module = parse_module_unchecked(&roundtrip)
            .unwrap_or_else(|err| panic!("reparse failed for {}: {}", path.display(), err));

        let mut right_units: Vec<_> = roundtrip_module.units().collect();
